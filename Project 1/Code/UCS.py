...
def search() -> Tuple[List[List[Tuple[str, int]]], int, int]:
    ...
    frontier: List[Tuple[int, State]] = [(0, initial_state)]
    reached: Dict[State, int] = {initial_state: 0}
    nodes_explored = 1

    while frontier:
        curr_cost, current_state = heappop(frontier)
        if current_state.at_goal_position():
            return current_state.get_trace(), nodes_explored, reached[current_state]
        next_states: List[State] = current_state.get_next_states()
        for next_state in next_states:
            path_cost = next_state.get_cost() + curr_cost
            if next_state not in reached or path_cost < reached[next_state]:
                nodes_explored += 1
                reached[next_state] = path_cost
                heappush(frontier, (path_cost, next_state))

    return [], nodes_explored, 0


def run_UCS() -> Tuple[List[List[Tuple[str, int]]], int, int]:
